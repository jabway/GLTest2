...
VertexBufferObject::VertexBufferObject(const vector<GLfloat> BufferData)
{
    Target = GL_ARRAY_BUFFER;

    Generate();
    Bind();
    glNamedBufferDataEXT(BufferId, BufferData.size()*sizeof(GLfloat), &BufferData[0], GL_STATIC_DRAW);
